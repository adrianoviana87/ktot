...
void deserialize(json &j, ktot::task_t &task) {
  ...
  task.external_id(j["externalId"].get<std::string>());
  ...
  std::sort(std::begin(works), std::end(works),
            [&](auto &i, auto &j) { return i->startedAt() < j->startedAt(); });
...
namespace ktot {
task_json_repository_t::task_json_repository_t(app_settings_ptr settings)
    : m_settings(settings) {}
...
std::vector<task_ptr> task_json_repository_t::list() {
  ...
  std::sort(std::begin(tasks), std::end(tasks), [](auto &i, auto &j) {
    return i->works().size() == 0 || j->works().size() == 0 ||
           i->works()[i->works().size() - 1]->startedAt() <
               j->works()[j->works().size() - 1]->startedAt();
  });
